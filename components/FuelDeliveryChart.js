// components/FuelDeliveryChart.js — With working "View all Months and Years" toggle
...
useEffect(() => {
  ...
  <Text style={{ color: 'white', marginRight: 10 }}>7-Day Moving Average</Text>
  <Switch
    value={showMovingAvg}
    onValueChange={() => setShowMovingAvg(prev => !prev)}
    trackColor={{ false: '#767577', true: '#81b0ff' }}
    thumbColor={showMovingAvg ? '#4B9CD3' : '#ccc'}
  />
</View>
              ...
              {visibleTypes.flatMap((type) => {
                ...
                return [
                  <VictoryLine
                    ...
                    labels={({ datum }) => `${formatLabel(type)}: ${datum.y.toFixed(2)}`}
                    labelComponent={
                      <VictoryTooltip
                        style={{ fontSize: 12, fontFamily: 'Segoe UI' }}
  ...
  <VictoryLine
    ...
    labels={({ datum }) => `7DMA: ${datum.y.toFixed(1)}`}
    labelComponent={
      <VictoryTooltip
        style={{ fontSize: 12, fontFamily: 'Segoe UI' }}
        constrainToVisibleArea
